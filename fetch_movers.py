#!/usr/bin/env python3
"""Fetch top market movers (US Yahoo Finance + A-shares Sina Finance) with news."""
...
def fetch(url, timeout=20, extra=None):
...
def get_us_movers(scr_id):
    ...
    try:
        data = json.loads(fetch(url, extra={"Accept": "application/json"}))
...
# ── A-Shares — Sina Finance (新浪财经) ────────────────────────────────────────

SINA_HEADERS = {"Referer": "https://finance.sina.com.cn/"}


def get_cn_movers(kind):
    """Fetch A-share top movers from Sina Finance quotes API."""
    ...
    try:
        data = json.loads(fetch(url, extra=SINA_HEADERS))
        result = []
        for item in (data or []):
            sym = item.get("symbol", "")           # e.g. "sz000001"
            mkt = sym[:2] if len(sym) > 2 else "sh"
...
def get_cn_news(symbol, count=3):
    """Fetch A-share stock news from Sina Finance per-stock news API."""
    url = ("https://vip.stock.finance.sina.com.cn/quotes_service/api/json_v2.php"
...
def enrich_cn(stocks):
    for s in stocks:
        ...
        s["name_zh"] = s["name"]   # already Chinese
        news = get_cn_news(s["symbol"])
